...
<script>
    var showAlert = true; // Set a flag to control the alert
    ...
    function checkReservationStatus() {
        ...
        if (refreshMessage && showAlert) {
            ...
            // Set timeout untuk menyembunyikan alert setelah 5 detik (sesuaikan dengan kebutuhan Anda)
            setTimeout(function() {
    ...
    // After displaying the SweetAlert, set showAlert to false to prevent further alerts
    function hideAlert() {
        showAlert = false;
    }
</script>
...
<script>
        // Fungsi untuk melakukan auto-refresh setiap beberapa detik
        function autoRefresh() {
            location.reload(); // Refresh halaman
        }

        // Auto-refresh setiap 600 detik (sesuaikan dengan kebutuhan Anda)
        setInterval(autoRefresh, 600000); // 600.000 milidetik = 600 detik

        // Periksa status reservasi pada sisi klien
        document.addEventListener("DOMContentLoaded", function() {
            var now = new Date();
            var reservations = {!! json_encode($reservations) !!}; // Ambil data reservasi dari server

            // Loop melalui daftar reservasi
            reservations.forEach(function(reservation) {
                var timeOut = new Date(reservation.reservation_time_out);
                if (timeOut < now) {
                    // Jika waktu telah berlalu, ubah status menjadi "Done"
                    reservation.status = 0;
                }
            });

            // Di sini Anda dapat memperbarui tampilan untuk mencerminkan status yang diperbarui
        });
    </script>
